...
public class FreeDrawHelper {
    ...
    static boolean isAPoint(@NonNull List<Point> points) {
        ...
        for (int i = 1; i < points.size(); i++) {
            if (points.get(i - 1).x != points.get(i).x || points.get(i - 1).y != points.get(i).y) return false;
        }
    ...
    static void setupStrokePaint(Paint paint) {
        ...
        paint.setPathEffect(new ComposePathEffect(new CornerPathEffect(100f), new CornerPathEffect(100f)));
